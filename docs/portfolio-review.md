...
- Project cards were mouse-oriented clickable containers with hover-only hints. They now use compact, non-clickable cards with a separate **More information / En savoir plus** button styled as a navigation link. Every project has a shareable `/projects/:slug` page; the four recent additions have expanded case studies. The earlier summary modal has been replaced by these pages. A modal is used only to enlarge gallery images.
