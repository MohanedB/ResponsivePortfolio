...
- Project cards were mouse-oriented clickable containers with hover-only hints. They now use real links with a persistent **More information / En savoir plus** prompt. Every project has a shareable `/projects/:slug` page; the four recent additions have expanded case studies. The earlier summary modal has been replaced by these pages. A modal is used only to enlarge gallery images.
